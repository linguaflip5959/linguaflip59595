...
function getUserCardsFor(langCode) {
  try {
    return JSON.parse(localStorage.getItem("linguaflip_cards_" + langCode) || "[]");
  } catch (e) { return []; }
}

function setUserCardsFor(langCode, cards) {
  localStorage.setItem("linguaflip_cards_" + langCode, JSON.stringify(cards));
}
...
function saveUserCards() {
  setUserCardsFor(CURRENT_LANG.code, userCards);
}

function addUserCard(data, targetLang) {
  targetLang = targetLang || CURRENT_LANG.code;
  const word = (data.word || "").trim();
  const translation = (data.translation || "").trim();
  if (!word || !translation) { toast("Нужны и слово, и перевод"); return false; }

  const isCurrent = targetLang === CURRENT_LANG.code;
  const targetDeck = isCurrent ? userCards : getUserCardsFor(targetLang);
  const baseDeck = isCurrent ? allCards : (LANGUAGES[targetLang] || {}).cards || [];

  const dup = baseDeck.concat(targetDeck)
    .some((c) => normalizeAnswer(c.word) === normalizeAnswer(word));
  if (dup) { toast("«" + word + "» уже есть в колоде"); return false; }
  ...
  if (isCurrent) {
    saveUserCards();
    renderTopicChips();
    if (currentTopic === "my") { updateFilteredCards(); renderCard(); saveProgress(); }
    toast("Добавлено: " + word + " → " + translation);
  } else {
    setUserCardsFor(targetLang, targetDeck);
    toast("Добавлено в колоду «" + ((LANGUAGES[targetLang] || {}).name || targetLang) + "»");
  }
  return true;
}

function removeUserCard(word) {
  const norm = normalizeAnswer(word);
  const i = userCards.findIndex((c) => normalizeAnswer(c.word) === norm);
  if (i === -1) { toast("Такого слова в «Моих» нет"); return; }
  const exact = userCards[i].word;
  userCards.splice(i, 1);
  saveUserCards();
  if (progress.marks[exact]) delete progress.marks[exact];
  saveProgress();
  updateStatsModal();
  renderTopicChips();
  updateFilteredCards();
  if (cardIdx >= filteredCards.length) cardIdx = Math.max(0, filteredCards.length - 1);
  renderCard();
  toast("Удалено: " + exact);
}
...
function editUserCard(oldWord, data) {
  const i = userCards.findIndex((c) => normalizeAnswer(c.word) === normalizeAnswer(oldWord));
  if (i === -1) { toast("Слова нет в «Моих»"); return false; }
  const word = (data.word || "").trim();
  const translation = (data.translation || "").trim();
  if (!word || !translation) { toast("Нужны и слово, и перевод"); return false; }

  const newNorm = normalizeAnswer(word);
  const dup = allCards.some((c) => normalizeAnswer(c.word) === newNorm) ||
    userCards.some((c, idx) => idx !== i && normalizeAnswer(c.word) === newNorm);
  if (dup) { toast("«" + word + "» уже есть в колоде"); return false; }

  const oldExact = userCards[i].word;
  userCards[i] = {
    topic: "my",
    word: word,
    transcription: (data.transcription || "").trim(),
  ...
  if (progress.marks[oldExact] && !progress.marks[word]) progress.marks[word] = progress.marks[oldExact];
  delete progress.marks[oldExact];

  renderTopicChips();
  updateFilteredCards();
  renderCard();
  saveProgress();
  toast("Обновлено: " + word);
  return true;
}
...
function renderTopicChips() {
  ...
  topics.forEach((t) => {
    ...
    if (
      ...
      t.key !== "all" &&
      t.key !== "difficult" &&
      t.key !== "my" // «Мои» растут — постоянная галочка тут наврала бы
    ) {
      classes += " completed";
    }
    ...
    if (classes.includes("completed")) {
      ...
      chip.innerHTML = `${t.label} <small>${userCards.length}</small>`;
    } else {
...
function showSessionComplete() {
  vibrate(10);
  document.getElementById("card-edit-btn").classList.remove("visible");
...
function updateCardContent() {
  ...
  const editBtn = document.getElementById("card-edit-btn");
  if (editBtn) editBtn.classList.toggle("visible", c.topic === "my");
...
/* ===== МОДАЛКА «ДОБАВИТЬ СЛОВО» (ФАЗА 0) ===== */
const addwordBtnModal = document.getElementById("addword-btn-modal");
const addwordModal = document.getElementById("addword-modal");
const addwordCloseBtn = document.getElementById("addword-close");
const addwordSaveBtn = document.getElementById("addword-save");
let editingWord = null;
const addwordFields = [
  document.getElementById("addword-word"),
  document.getElementById("addword-translation"),
  document.getElementById("addword-transcription"),
  document.getElementById("addword-example"),
  document.getElementById("addword-example-ru"),
];

const addwordDeleteBtn = document.getElementById("addword-delete");
const addwordLangSelect = document.getElementById("addword-lang");

if (addwordLangSelect) {
  Object.values(LANGUAGES).forEach((l) => {
    const opt = document.createElement("option");
    opt.value = l.code;
    opt.textContent = l.name;
    addwordLangSelect.appendChild(opt);
  });
}

function openAddWordModal(card) {
  if (!addwordModal || !addwordFields[0]) return;
  editingWord = card ? card.word : null;

  const title = document.getElementById("addword-title");
  if (title) title.textContent = card ? "Редактировать слово" : "Новое слово";
  if (addwordSaveBtn) addwordSaveBtn.textContent = card ? "Сохранить" : "Добавить в «Мои»";
  if (addwordDeleteBtn) addwordDeleteBtn.style.display = card ? "block" : "none";
  if (addwordLangSelect) {
    addwordLangSelect.value = CURRENT_LANG.code;
    addwordLangSelect.disabled = !!card;
  }

  const vals = card
    ? [card.word, card.translation, card.transcription, stripHtml(card.example), card.exampleTranslation]
    : ["", "", "", "", ""];
  addwordFields.forEach((inp, idx) => {
    inp.value = vals[idx] || "";
    inp.classList.remove("incorrect");
  });

  addwordModal.classList.add("show");
  setTimeout(() => addwordFields[0].focus(), 350);
}

if (addwordBtnModal && statsModal) {
  addwordBtnModal.addEventListener("click", () => {
    statsModal.classList.remove("show");
    setTimeout(openAddWordModal, 300);
  });
}

if (addwordCloseBtn) {
  addwordCloseBtn.addEventListener("click", () =>
...
if (addwordSaveBtn) {
  addwordSaveBtn.addEventListener("click", () => {
    const data = {
      word: addwordFields[0].value,
      translation: addwordFields[1].value,
      transcription: addwordFields[2].value,
      example: addwordFields[3].value,
      exampleTranslation: addwordFields[4].value,
    };

    const ok = editingWord
      ? editUserCard(editingWord, data)
      : addUserCard(
          data,
          addwordLangSelect ? addwordLangSelect.value : CURRENT_LANG.code,
        );
    ...
    if (ok) {
      ...
      addwordFields[0].classList.add("incorrect");
      setTimeout(() => addwordFields[0].classList.remove("incorrect"), 1000);
    }
  });

  addwordFields.forEach((inp) => {
    if (!inp) return;
    inp.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        addwordSaveBtn.click();
      }
    });
  });
}
...
if (addwordDeleteBtn) {
  addwordDeleteBtn.addEventListener("click", () => {
    if (!editingWord) return;
    removeUserCard(editingWord);
    addwordModal.classList.remove("show");
    editingWord = null;
  });
}
